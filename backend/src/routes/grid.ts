...
async function collectGridReferenceAssets(storyboards: any[]) {
  ...
  const dramaId = storyboards[0]?.dramaId ?? null

  const scenes = sceneIds.length && dramaId
    ? (await listScenesByDramaId(dramaId)).filter((scene) => sceneIds.includes(scene.id))
    : []
  const characters = characterIds.length && dramaId
    ? (await listCharactersByDramaId(dramaId)).filter((char) => characterIds.includes(char.id))
    : []
...
app.post('/prompt', async (c) => {
  ...
  const storyboards = (await Promise.all(storyboard_ids.map((id: number) => getStoryboardById(id)))).filter(Boolean)

  if (!storyboards.length) return badRequest(c, 'No storyboards found')

  let dramaStyle = ''
  if (drama_id) {
    const drama = await getDramaById(drama_id)
...
app.post('/split', async (c) => {
  ...
  const imgRecord = await getImageGenerationById(image_generation_id)
  ...
  try {
    ...
    for (let i = 0; i < assignments.length && i < cells.length; i++) {
      ...
      if (frame_type === 'first_frame') update.firstFrameImage = cell.localPath
        ...
        const sb = await getStoryboardById(storyboard_id)
      ...
      await updateStoryboard(storyboard_id, update)

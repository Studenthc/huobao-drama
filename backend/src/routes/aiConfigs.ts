...
import {
  ...
  findAiServiceConfigByServiceTypeAndProvider,
  getAgentConfigByType,
  getAiServiceConfigById,
  listAgentConfigs,
  listAiServiceConfigs,
  listAiServiceProviders,
  updateAgentConfig,
...
app.get('/', async (c) => {
  ...
  let rows = await listAiServiceConfigs()
...
app.post('/', async (c) => {
  ...
  const row = await createAiServiceConfig({
...
app.post('/huobao-preset', async (c) => {
  ...
  for (const preset of HUOBAO_PRESET_SERVICES) {
    const existing = await findAiServiceConfigByServiceTypeAndProvider(preset.serviceType, preset.provider)
    ...
    if (existing) {
      await updateAiServiceConfig(existing.id, values)
    } else {
      await createAiServiceConfig({
        ...values,
        createdAt: ts,
      })
    }
  ...
  for (const agent of HUOBAO_AGENT_DEFAULTS) {
    const existing = await getAgentConfigByType(agent.agentType)
    const values = {
      name: agent.name,
    ...
    if (existing) {
      await updateAgentConfig(existing.id, values)
    } else {
  ...
  const configs = (await listAiServiceConfigs()).map(row => ({
    ...toSnakeCase(row),
    model: row.model ? JSON.parse(row.model) : [],
  }))
  const agents = (await listAgentConfigs()).map(row => toSnakeCase(row))
...
app.put('/:id', async (c) => {
  ...
  await updateAiServiceConfig(id, updates)
  return success(c)
})
...
aiProviders.get('/', async (c) => {
  const rows = await listAiServiceProviders()
  const parsed = rows.map(r => ({
    ...toSnakeCase(r),

...
import {
  ...
  listScenesByDramaId,
  listStoryboardCharacterLinksByStoryboardId,
  listStoryboardsByEpisodeIdOrdered,
  replaceStoryboardCharacters,
  createStoryboard,
  updateEpisode,
  updateStoryboard as updateStoryboardRecord,
} from '../../db/repos/studio-content.js'

async function syncStoryboardCharacters(storyboardId: number, characterIds: number[]) {
  await replaceStoryboardCharacters(storyboardId, characterIds)
}

async function getEpisodeSceneIds(episodeId: number) {
  return new Set(
    (await listEpisodeSceneLinks(episodeId)).map(link => link.sceneId),
  )
}

async function getEpisodeCharacterIds(episodeId: number) {
  return new Set(
    (await listEpisodeCharacterLinks(episodeId)).map(link => link.characterId),
  )
}
...
export function createStoryboardTools(episodeId: number, dramaId: number) {
  const readStoryboardContext = createTool({
    ...
    execute: async () => {
      ...
      const charLinks = await listEpisodeCharacterLinks(episodeId)
      const sceneLinks = await listEpisodeSceneLinks(episodeId)
      ...
      const scns = await listScenesByDramaId(dramaId)
      const existingStoryboards = await listStoryboardsByEpisodeIdOrdered(episodeId)
      ...
      const existingStoryboardsPayload = await Promise.all(
        existingStoryboards
          ...
          .map(async (sb) => ({
            ...
            title: sb.title || '',
            scene_id: sb.sceneId,
            character_ids: (await listStoryboardCharacterLinksByStoryboardId(sb.id))
      ...
      const payload = {
        ...
        characters,
        scenes,
        existing_storyboards: existingStoryboardsPayload,
      }
  ...
  const saveStoryboards = createTool({
    ...
    execute: async ({ storyboards }) => {
      ...
      const existingStoryboardIds = (await listStoryboardsByEpisodeIdOrdered(episodeId))
        .map(sb => sb.id)
      for (const storyboardId of existingStoryboardIds) {
        await replaceStoryboardCharacters(storyboardId, [])
      }
      await deleteStoryboardsByEpisodeId(episodeId)
      ...
      for (const sb of storyboards) {
        ...
        await syncStoryboardCharacters(created.id, sb.character_ids || [])
        totalDuration += sb.duration || 10
      }

      await updateEpisode(episodeId, { duration: Math.ceil(totalDuration / 60), updatedAt: ts })
  ...
  const updateStoryboard = createTool({
    ...
    execute: async ({ storyboard_id, ...fields }) => {
      const storyboard = await getStoryboardById(storyboard_id)
      ...
      await validateStoryboardBindings(
        ...
        'character_ids' in fields
          ...
          : (await listStoryboardCharacterLinksByStoryboardId(storyboard_id))
              .map(link => link.characterId),
      )
      ...
      await updateStoryboardRecord(storyboard_id, updates)

...
import { createNeonSqlClient, hasNeonDatabaseUrl } from './neon-client.js'
...
export const dbMode = hasNeonDatabaseUrl() ? 'postgres' : 'sqlite'
export const schema = (dbMode === 'postgres' ? pgSchema : sqliteSchema) as typeof sqliteSchema
...
if (dbMode === 'postgres') {
  ...
  dbInstance = drizzlePg(sql, { schema: pgSchema })
} else {
  sqlite = new Database(DB_PATH, { timeout: 30000 })
...
export const db = dbInstance as ReturnType<typeof drizzleSqlite<typeof sqliteSchema>>
...
export async function queryAll(query: any): Promise<any[]> {
  return isPostgresDb() ? await query : query.all()
}

export async function queryFirst(query: any): Promise<any | null> {
  const rows = await queryAll(query)
  return rows[0] ?? null
}

export async function executeWrite(query: any) {
  return isPostgresDb() ? await query : query.run()
}

export async function insertAndReturnOne(table: any, values: any, idColumn: any) {
  if (isPostgresDb()) {
  ...
  return queryFirst(
    db.select().from(table).where(eq(idColumn, Number(result.lastInsertRowid))),
  )
}
